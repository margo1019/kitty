 ...
 */
public class LockEfficiencyTest extends Thread {

    //整体表现最好,短时间的低并发下比AtomicInteger性能差一点，高并发下性能最高；
    private static LongAdder longAdder = new LongAdder();

    //短时间低并发下，效率比synchronized高，甚至比LongAdder还高出一点，但是高并发下，性能还不如synchronized；不同情况下性能表现很不稳定；
    ...
    static {
        try {
            ...
            casOffset = UNSAFE.staticFieldOffset(LockEfficiencyTest.class.getDeclaredField("cas"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void cas() {
        ...
        while (!bl) {
            tmp = cas;
            bl = UNSAFE.compareAndSwapInt(LockEfficiencyTest.class, casOffset, tmp, tmp + 1);
        }
    }
